...
The script `.run/scripts/camel-springboot-matrix.sh` generates an AsciiDoc compatibility matrix
for Apache Camel Spring Boot and Spring Boot versions, including Apache CXF versions and release dates.
...
----
# All versions
bash .run/scripts/camel-springboot-matrix.sh

# With version range (min max)
bash .run/scripts/camel-springboot-matrix.sh 4.0.0 4.15.0
----

The script writes the result to `camel-springboot-matrix.adoc` in the current directory.
...
| Camel Version
| The Camel Spring Boot release version

| Type
| Release type from camel.apache.org/releases (PATCH / MINOR / MAJOR / LTS)

| Camel Release Date
| Date from Maven Central directory listing

| Spring Boot Version
| Compatible Spring Boot version read from the Camel POM

| Spring Boot Release Date
| Date from Maven Central directory listing

| CXF Version
| Apache CXF version from `camel-parent` POM (`<cxf-version>`)

| CXF POM URL / Spring Boot POM URL
| Direct links to the source POM files on Maven Central
|===
